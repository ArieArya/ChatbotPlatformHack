...
urlpatterns = [
    ...
    path('query&topCount=<int:n>&timePeriod=<int:time>&unit=<str:period>', v.getNCount),
    
    # Obtains the top "n" coins and their total score in past time period
    path('query&topScore=<int:n>&timePeriod=<int:time>&unit=<str:period>', v.getNScore),
    
    # Obtains total count and score of coin "crypto_symbol" in the past time period
    path('query&coinTotalCountScore=<str:crypto_symbol>&timePeriod=<int:time>&unit=<str:period>',
         v.getCoinCountScoreTotal),
    
    # Obtains all hourly data of top "n" coin in the past time period
    path('query&hourlyData=<int:n>&timePeriod=<int:time>&unit=<str:period>',
         v.getAllTopData),
    
    # Obtains all hourly data of coin "crypto_symbol" in the past time period
    path('query&coinHourlyData=<str:crypto_symbol>&timePeriod=<int:time>&unit=<str:period>',
         v.getAllDataCoin),
    
    # Obtains combined data for top "n" coins in the past time period
    path('query&hourlyData=<int:n>&timePeriod=<int:time>&unit=<str:period>&combinedData=<int:combined_data>',
         v.getAllTopDataCombinedHours),
    
    # Obtains combined data for coin "crypto_symbol" in the past time period
    path('query&coinHourlyData=<str:crypto_symbol>&timePeriod=<int:time>&unit=<str:period>&combinedData=<int:combined_data>',
